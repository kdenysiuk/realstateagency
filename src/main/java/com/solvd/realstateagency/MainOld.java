...
public class MainOld {
	...
	public static void main(String[] args) throws InvalidOptionException {
		try (Scanner input = new Scanner(System.in)) {
			...
			//I set some amount of money for eac)h person
			try {
			...
			//arrBuilding.forEach((Building elem) -> propertiesAvailable.add(elem));
			//arrBuilding.filter((elem) -> cust1.getSalary() * 0.5 >= elem.getRentPrice());
			...
			propertiesAvailable.forEach((Building elem) -> LOGGER.info("Adress: "+elem.getAdress()+" Price per month: AR$ "+elem.getRentPrice()));
		}
